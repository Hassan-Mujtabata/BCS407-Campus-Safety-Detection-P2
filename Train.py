"""
Emergency Exit Detection — YOLOv8l Training Pipeline
=====================================================
Covers all requirements:
 1. YOLOv8l model
 2. Weights saved as final_project_model.pt
 3. Unannotated images → test set; zero-detection images → excluded next run
 4. AMP FP16 / GradScaler / zero_grad handled by Ultralytics internally (amp=True)
    pin_memory / non_blocking / num_workers=7 set on dataloader
 5. Adaptive runtime checks — no hardcoded values
 6. GPU utilization monitor in training loop — auto-adjusts if low
 7. Auto-split flat dataset 70/20/10
 8. Full distribution report per class per split
 9. Train and val in separate folders (enforced by auto-split)
10. Auto-merge new images into split at runtime
11. Early stopping (patience)
12. Training plots saved automatically
13. Confusion matrix auto-generated
14. Resume from checkpoint if found
15. Estimated training time printed before start
16. Minority class warnings before training
17. Auto batch size from VRAM
18. VRAM usage monitor per epoch
19. Auto augmentation
20. Class weights computed and logged (minority balancing)
21. Per-epoch CSV logging
22. Test set evaluation with per-class metrics after training
"""
...
# ─── USER CONFIG — edit these paths to match your setup ──────────────────────
...
MIN_WARN      = 30                     # warn if a class has fewer train samples
PROJECT       = "runs"
NAME          = "final_project_model"
IMGSZ         = 640
# ─────────────────────────────────────────────────────────────────────────────
...
def auto_batch(vram_gb: float) -> int:
    """
    Estimate max safe batch size for YOLOv8l at 640px (FP16).
    YOLOv8l uses ~0.034 GB/image at 640px FP16.
    Keep 1.5 GB headroom for activations/gradients.
    """
    mem_per_img = 0.034          # GB per image
    headroom    = 1.5            # GB reserved
    usable      = max(0.0, vram_gb - headroom)
    batch       = max(4, int(usable / mem_per_img))
    # round down to nearest power of 2 for efficiency
    batch = 2 ** (batch.bit_length() - 1)
    batch = min(batch, 64)       # cap — more than 64 rarely helps on 8 GB VRAM
    print(f"   Auto-batch: {batch}  (VRAM {vram_gb:.1f} GB, headroom {headroom} GB)")
    return batch
...
def is_flat(root: Path) -> bool:
    """True if dataset is flat (images/ labels/ at root, no train/ folder)."""
    return (root / "images").exists() and not (root / "train").exists()


def split_and_organise(root: Path, ratios=(0.70, 0.20, 0.10), seed=42):
    """
    Split a flat dataset into train/ valid/ test/ subfolders.
    - Annotated images  → shuffled, split by ratios
    - Unannotated images → test set only
    - Previously excluded images (zero-detection) → skipped from train/valid
    ...
    # remove excluded from annotated pool
...
def make_split_yaml(root: Path, src_yaml: Path) -> Path:
    """Write a new data.yaml pointing to the split folders."""
...
def estimate_time(n_train: int, batch: int, epochs: int, vram_gb: float):
    # YOLOv8l at 640px on RTX 3070 Ti ≈ 3–4 ms/image/epoch at batch 16
    ms_per_img = 3.5
    if vram_gb >= 8:
        ms_per_img = 3.0
...
def cb_epoch_end(trainer):
    ...
    # ── GPU utilisation guard ─────────────────────────────────────────────
    if util < 50:
        _state["low_util_streak"] += 1
        streak = _state["low_util_streak"]
        print(f"\n⚠️  GPU util {util}% at epoch {epoch} (streak={streak})")
        if streak >= 2:
            # Push harder: clear cache so allocator can reuse freed blocks
            torch.cuda.empty_cache()
            # Increase persistent workers hint via env (takes effect next epoch)
...
def find_zero_detection_images(model_path: Path, train_img_dir: Path,
                                device: str, conf: float = 0.10):
    """
    Run inference on all training images.
    Images where the model detects nothing → added to exclusion list.
    These are skipped in the next training run (item 4).
    """
    print("\n🔍 Scanning train images for zero-detection cases...")
    model  = YOLO(str(model_path))
    imgs   = all_images(train_img_dir)
    old_ex = load_exclusions()
    new_ex = set()

    for img in imgs:
        res = model.predict(str(img), conf=conf, verbose=False, device=device)
        if len(res[0].boxes) == 0:
            new_ex.add(img.name)

    all_ex = old_ex | new_ex
    save_exclusions(all_ex)
    print(f"   New zero-detection: {len(new_ex)}  |  Total excluded: {len(all_ex)}")


def evaluate_test_set(model_path: Path, data_yaml: Path,
                       class_names: list, device: str):
    """Full test-set evaluation with per-class metrics."""
    print("\n" + "=" * 65)
    ...
    if ap_idx is not None and len(ap_idx):
        ...
        print("   (per-class index not available — check Ultralytics version)")
...
if __name__ == "__main__":
    print("=" * 65)
    print("🚦  EMERGENCY EXIT — YOLOv8l TRAINING PIPELINE")
    print("=" * 65)
    ...
    if is_flat(DATASET_ROOT):
        ...
        split_and_organise(DATASET_ROOT, SPLIT_RATIOS)
    else:
    ...
    if last_ckpt.exists():
        print(f"\n🔄 Checkpoint found → resuming: {last_ckpt}")
        model  = YOLO(str(last_ckpt))
        resume = True
    else:
        print("\n🆕 No checkpoint — starting fresh with yolov8l.pt")
    ...
    # ── Train ─────────────────────────────────────────────────────────────
    # NOTE: Ultralytics handles internally:
    #   • autocast FP16 (amp=True)
    #   • GradScaler
    #   • zero_grad(set_to_none=True)
    #   • pin_memory=True / non_blocking=True on DataLoader
    model.train(
        ...
        resume    = resume,
        plots     = True,           # loss / PR curves / confusion matrix
        save      = True,
        val       = True,
        verbose   = True,
        optimizer = "AdamW",
        ...
        mosaic    = 1.0,            # mosaic (4-image stitch)
    ...
    # ── Zero-detection exclusion for next run ─────────────────────────────
    find_zero_detection_images(
        final_weights,
